...
export default function ServiceConnectSignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.phone || !formData.password) {
      const message = "Please fill in name, email, phone, and password. Phone number is required.";
      setStatusType("error");
    ...
    if (isWorker && (!formData.tradeCategory || !formData.experienceYears || !formData.serviceArea || formData.phone.trim() === "")) {
  ...
  const handleGoogleSignup = () => {
    ...
    if (!formData.phone.trim()) {
      toast.error("Phone number is required before continuing with Google signup.");
      return;
    }
    if (isWorker) {
    ...
    const pendingData = isWorker
      ...
      : { role: "CUSTOMER", phone: formData.phone };
